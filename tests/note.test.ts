...
test('A user can create a note', async () => {
  ...
  const response = await request.post('/notes/create').set('Authorization', token).send(payload);
...
test('A user can update a note', async () => {
  ...
  const response = await request.put(`/notes/${note.id}/update`).set('Authorization', token).send(payload);
...
test('A user can get a note', async () => {
  ...
  const response = await request.get(`/notes/${note.id}`).set('Authorization', token);
...
test('A user can delete a note', async () => {
  ...
  const response = await request.delete(`/notes/${note.id}/delete`).set('Authorization', token);
